...
export default function AdminEditPage() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      await fetchServices();
      if (match && params?.id) {
        fetchProfile(parseInt(params.id), provincesData.data);
      }
    };
    init();
  ...
  const fetchProfile = async (id: number, provincesData: Dictionary[]) => {
    setLoading(true);
    try {
      ...
      // 设置省份和城市
      if (data.province) {
        const provinceData = provincesData.find((p) => p.dictValue === data.province);
        if (provinceData) {
          ...
          try {
            ...
            if (citiesResponse.ok) {
              ...
              if (data.city) {
                const cityData = citiesData.find((c: Dictionary) => c.dictValue === data.city);
                if (cityData) {
    ...
    <div className="min-h-screen bg-slate-50 pb-20">
      <header className="sticky top-0 z-50 bg-white border-b border-slate-200 shadow-sm">
        <div className="container mx-auto px-4 flex items-center h-16">
          ...
          <h1 className="ml-4 text-xl font-bold text-slate-900">编辑用户</h1>
        </div>
      </header>
          ...
          <div className="md:col-span-1 space-y-6">
            ...
            <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm">
              ...
              <p className="text-[10px] text-slate-400 leading-relaxed">
                提示：支持批量上传图片和视频。首个媒体资源将作为详情页默认展示。
              </p>
            </div>
          </div>
